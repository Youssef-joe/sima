...
app = FastAPI(title="SIMA API v4.2 + Upgrade Pack 1")
...
class RegisterIn(BaseModel): email:str; full_name:str; role:str=Field(..., pattern="^(authority|consultant|client)$"); password:str
class LoginIn(BaseModel): email:str; password:str
class ProjectIn(BaseModel): title:str; region:str; function:str; city:str
    ...
    with db() as c:
        with c.cursor() as cur:
            for it in items:
                ...
                vec = embed(it["text"], dim=int(os.getenv("EMBED_DIM","256")))
                cur.execute("""
                  INSERT INTO rag_docs (id,title,content,embedding)
                  VALUES (%s,%s,%s,%s)
                  ON CONFLICT (id) DO NOTHING
                """, (eid, f"{region} guideline", it["text"], vec))
