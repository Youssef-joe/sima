...
export default function Login(){
  ...
  async function ensureSeedUser(){
    try{
      await fetch('http://localhost:8080/auth/register',{method:'POST',headers:{'Content-Type':'application/json'},
        body: JSON.stringify({email, full_name:'Authority', role, password})});
    }catch{}
  }
  ...
  async function login(e:any){
    e.preventDefault();
    const r = await fetch('http://localhost:8080/auth/login',{method:'POST',headers:{'Content-Type':'application/json'},
